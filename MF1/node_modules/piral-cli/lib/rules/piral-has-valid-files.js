...
function hasSubdirectories(target) {
    if (fs_1.statSync(target).isDirectory()) {
        return fs_1.readdirSync(target).some((name) => fs_1.statSync(path_1.join(target, name)).isDirectory());
    }
    return false;
}
/**
...
`);
        ...
        for (const file of validFileRefs) {
            ...
            const target = path_1.join(context.root, from);
...
`);
            ...
            else if (ignoredFiles.includes(path_1.basename(target))) {
                context.error(`
