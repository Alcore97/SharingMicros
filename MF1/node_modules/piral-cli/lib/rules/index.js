...
function getRules(target) {
    ...
    return new Promise((resolve, reject) => {
        fs_1.readdir(__dirname, (err, files) => {
            if (err) {
                reject(err);
            }
            else {
                resolve(files
                    ...
                    .map((fileName) => {
                    common_1.log('generalDebug_0003', `Including module "${fileName}" ...`);
                    const run = require(`./${fileName}`).default;
                    const name = fileName.substr(prefix.length).replace(/\.js$/, '');
                    common_1.log('generalDebug_0003', `Included rule with name: "${name}".`);
                    return {
                        run,
                        name,
                    };
...
function getPiralRules() {
    return __awaiter(this, void 0, void 0, function* () {
        common_1.log('generalDebug_0003', 'Getting Piral validation rules ...');
        const rules = yield getRules('piral');
        common_1.log('generalDebug_0003', `Found ${rules.length} rules.`);
...
function getPiletRules() {
    return __awaiter(this, void 0, void 0, function* () {
        common_1.log('generalDebug_0003', 'Getting pilet validation rules ...');
