...
function isProjectReference(name) {
    const target = path_1.resolve(name, 'package.json');
...
function detectNpm(root) {
    return new Promise((res) => {
        fs_1.access(path_1.resolve(root, 'package-lock.json'), fs_1.constants.F_OK, (noPackageLock) => {
            res(!noPackageLock);
        });
    });
}
    ...
    return __awaiter(this, void 0, void 0, function* () {
        ...
        if (file) {
            try {
                ...
                log_1.log('generalError_0002', `Could not read lerna.json: ${err}.`);
            }
        }
        return undefined;
    });
...
function determineNpmClient(root, selected) {
    return __awaiter(this, void 0, void 0, function* () {
        if (!selected || !helpers_1.clientTypeKeys.includes(selected)) {
            log_1.log('generalDebug_0003', 'No NPM client selected. Checking for lock files ...');
            ...
            log_1.log('generalDebug_0003', `Results of the lock file check: NPM = ${hasNpm}, Yarn = ${hasYarn}, Pnpm = ${hasPnpm}`);
            ...
            if (found !== 1) {
                ...
                if (helpers_1.clientTypeKeys.includes(lernaClient)) {
                    log_1.log('generalDebug_0003', `Found valid NPM client via Lerna: ${lernaClient}.`);
                    return lernaClient;
                }
            }
            else if (hasNpm) {
                log_1.log('generalDebug_0003', `Found valid NPM client via lockfile.`);
                return 'npm';
            }
            else if (hasYarn) {
                log_1.log('generalDebug_0003', `Found valid Yarn client via lockfile.`);
                return 'yarn';
            }
            else if (hasPnpm) {
            ...
            if (helpers_1.clientTypeKeys.includes(defaultClient)) {
                log_1.log('generalDebug_0003', `Found valid Pnpm client the default client: "${defaultClient}".`);
                return defaultClient;
            }
            log_1.log('generalDebug_0003', 'Using the default NPM client.');
            return 'npm';
        }
        return selected;
    });
...
function isMonorepoPackageRef(refName, root) {
    ...
    return __awaiter(this, void 0, void 0, function* () {
        ...
        const details = yield c.listPackage(refName, root);
...
function detectMonorepo(root) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        const packageJson = yield io_1.readJson(root, 'package.json');
...
function getPackageName(root, name, type) {
    return __awaiter(this, void 0, void 0, function* () {
        switch (type) {
            case 'file':
                const originalPackageJson = yield io_1.readJson(name, 'package.json');
...
function makeExternals(externals) {
    if (externals && Array.isArray(externals)) {
        ...
        const all = exclude.includes('*') ? include : [...include, ...constants_1.coreExternals];
        return all.filter((m, i, arr) => !exclude.includes(m) && arr.indexOf(m) === i);
    }
    return constants_1.coreExternals;
}
