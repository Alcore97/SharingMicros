...
function patchModule(packageName, rootDir) {
    return __awaiter(this, void 0, void 0, function* () {
        const applyPatchAt = patches_1.getPatch(packageName);
        if (typeof applyPatchAt === 'function') {
            log_1.log('generalDebug_0003', `Applying patchers for ${packageName} in "${rootDir}" ...`);
...
function patch(staticPath, ignoredPackages) {
    return __awaiter(this, void 0, void 0, function* () {
        log_1.log('generalDebug_0003', `Patching files in "${staticPath}" ...`);
        const folderNames = yield io_1.getFileNames(staticPath);
        return Promise.all(folderNames.map((folderName) => __awaiter(this, void 0, void 0, function* () {
            if (!ignoredPackages.includes(folderName)) {
                const rootName = path_1.resolve(staticPath, folderName);
                const isDirectory = yield io_1.checkIsDirectory(rootName);
                if (isDirectory) {
                    if (folderName.startsWith('@')) {
                        ...
                        try {
                            const packageFileData = yield io_1.readJson(rootName, 'package.json');
                            if (packageFileData.name && packageFileData._piralOptimized === undefined) {
                                ...
                                yield io_1.writeJson(rootName, 'package.json', packageFileData, true);
                                yield io_1.writeText(rootName, '.browserslistrc', 'node 10.11');
                                yield patchModule(folderName, rootName);
                            }
                            yield patchFolder(rootName, ignoredPackages);
                        }
                        catch (e) {
                            log_1.log('generalDebug_0003', `Encountered a patching error: ${e}`);
                        }
                    }
                }
            }
...
function patchFolder(rootDir, ignoredPackages) {
    return __awaiter(this, void 0, void 0, function* () {
        const file = '.patched';
        const modulesDir = path_1.resolve(rootDir, 'node_modules');
        const exists = yield io_1.checkExists(modulesDir);
        if (exists) {
            const lockContent = (yield io_1.readText(rootDir, 'package-lock.json')) || (yield io_1.readText(rootDir, 'yarn.lock'));
            const currHash = hash_1.computeHash(lockContent);
            const prevHash = yield io_1.readText(modulesDir, file);
            log_1.log('generalDebug_0003', `Evaluated patch module hashes: "${currHash}" and "${prevHash}".`);
            if (prevHash !== currHash) {
                yield patch(modulesDir, ignoredPackages);
                yield io_1.createFileIfNotExists(modulesDir, file, currHash, enums_1.ForceOverwrite.yes);
            }
        }
    });
}
function patchModules(rootDir, ignoredPackages = defaultIgnoredPackages) {
    return __awaiter(this, void 0, void 0, function* () {
        log_1.log('generalDebug_0003', `Patching modules starting in "${rootDir}" ...`);
        const otherRoot = path_1.resolve(require.resolve('piral-cli/package.json'), '..', '..', '..');
        yield patchFolder(rootDir, ignoredPackages);
        if (otherRoot !== rootDir) {
            log_1.log('generalDebug_0003', `Also patching modules in "${otherRoot}" ...`);
