...
exports.runCommand = exports.runScript = void 0;
...
function resolveWinPath(specialFolder, subPath) {
    ...
    if (basePath) {
        return path_1.resolve(basePath, subPath);
    }
    return undefined;
}
function runScript(script, cwd = process.cwd(), output = process.stdout) {
    ...
    log_1.log('generalDebug_0003', `Running "${script}" in "${cwd}" ("${bin}").`);
    if (info_1.isWindows) {
...
function runCommand(exe, args, cwd, output) {
    ...
    log_1.log('generalDebug_0003', `Applying cmd "${cmd}" in directory "${cwd}".`);
