...
function matchAnyPilet(baseDir, patterns) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        yield Promise.all(allPatterns.map((patterns) => matchAnyPattern(baseDir, patterns).then(({ results, pattern }) => __awaiter(this, void 0, void 0, function* () {
            if (!results.length) {
                log_1.log('generalDebug_0003', `Found no potential entry points using "${pattern}".`);
            }
            else {
                log_1.log('generalDebug_0003', `Found ${results.length} potential entry points in "${pattern}".`);
                for (const result of results) {
                    const fileName = path_1.basename(result);
                    if (fileName === nameOfPackageJson) {
                        log_1.log('generalDebug_0003', `Entry point is a "${nameOfPackageJson}" and needs further inspection.`);
                        const targetDir = path_1.dirname(result);
                        ...
                        if (packageJson) {
                            const targetDir = path_1.dirname(packageJson);
                            const { name } = yield readJson(targetDir, nameOfPackageJson);
                            if (!pilets.includes(name)) {
                                log_1.log('generalDebug_0003', `Entry point result is "${result}".`);
                                matched(name, result);
                            }
                        }
                        else {
                            log_1.log('generalDebug_0003', `Could not find "${nameOfPackageJson}" for entry "${result}". Skipping.`);
                        }
                    }
                }
            }
