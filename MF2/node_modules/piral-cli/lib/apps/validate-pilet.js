...
function validatePilet(baseDir = process.cwd(), options = {}) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        const rules = yield rules_1.getPiletRules();
        const entryFile = path_1.join(baseDir, entry);
        const target = path_1.dirname(entryFile);
        const _a = yield common_1.retrievePiletData(target, app), { dependencies, peerDependencies, devDependencies, peerModules, root, ignored: _0, emulator: _1 } = _a, data = __rest(_a, ["dependencies", "peerDependencies", "devDependencies", "peerModules", "root", "ignored", "emulator"]);
        const { validators } = common_1.getPiletsInfo(data.appPackage);
        const errors = [];
        const warnings = [];
        const context = {
            error(message) {
                errors.push(common_1.log('generalError_0002', message));
            },
            warning(message) {
                warnings.push(common_1.log('generalWarning_0001', message));
            },
            logLevel,
            entry: entryFile,
            dependencies,
        ...
        yield common_1.runRules(rules, context, validators);
