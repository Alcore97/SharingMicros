...
function createTarball(sourceDir, targetDir, targetFile) {
    const folder = path_1.relative(targetDir, sourceDir);
    log_1.log('generalDebug_0003', `Create archive "${targetFile}" in "${targetDir}" containing "${folder}".`);
    return external_1.tar.create({
        file: path_1.resolve(targetDir, targetFile),
        cwd: targetDir,
    }, [folder]);
}
exports.createTarball = createTarball;
function unpackTarball(sourceDir, sourceFile) {
    log_1.log('generalDebug_0003', `Extract files from "${sourceFile}" in "${sourceDir}".`);
    return external_1.tar.extract({
        file: path_1.resolve(sourceDir, sourceFile),
        keep: false,
        cwd: sourceDir,
    });
}
...
function unpackGzTar(stream) {
    ...
    return new Promise((resolve, reject) => {
        const files = {};
        log_1.log('generalDebug_0003', `Unpacking the stream ...`);
        stream
            .on('error', reject)
            ...
            const content = [];
            const p = e.path;
            log_1.log('generalDebug_0003', `Found entry "${p}" in packed content.`);
            e.on('error', reject);
