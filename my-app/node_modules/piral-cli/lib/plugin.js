...
function listDirectory(rootDir) {
    return new Promise((resolve) => {
        fs_1.readdir(rootDir, (_, files) => resolve(files || []));
    });
}
function isPluginDirectory(dir) {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            ...
            common_1.log('generalDebug_0003', `Could not load the plugin at "${dir}": ${err}`);
            return false;
        }
    });
}
...
function getAllPlugins(rootDir) {
    return __awaiter(this, void 0, void 0, function* () {
        if (rootDir) {
            common_1.log('generalDebug_0003', `Getting plugins from dir "${rootDir}" ...`);
            const pluginPaths = yield listDirectory(rootDir);
            const plugins = [];
            const nested = pluginPaths.filter(isScope).map((m) => path_1.join(rootDir, m));
            yield Promise.all([
                fillPlugins(pluginPaths.filter(isPlugin).map((m) => path_1.join(rootDir, m)), plugins),
                ...nested.map((path) => __awaiter(this, void 0, void 0, function* () {
                    const files = yield listDirectory(path);
                    yield fillPlugins(files.filter(isPlugin).map((m) => path_1.join(path, m)), plugins);
                })),
            ]);
            return plugins;
        }
        else {
            common_1.log('generalDebug_0003', `Skipping plugins from dir "${rootDir}" ...`);
            return [];
        }
    });
}
